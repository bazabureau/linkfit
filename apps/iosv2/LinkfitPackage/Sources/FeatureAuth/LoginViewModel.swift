...
public final class LoginViewModel {
    ...
    public var canSubmit: Bool {
        AuthValidation.isValidEmail(email) && AuthValidation.isValidPassword(password) && !isSubmitting
    }

    public func submit() async {
        guard canSubmit else { return }
        await run { try await self.repository.login(email: self.email.trimmedValue, password: self.password) }
    }
    ...
    private func applyAPIError(_ api: APIError) {
        if case .validation(let fields) = api, !fields.isEmpty {
            ...
            if emailError == nil && passwordError == nil { errorMessage = api.userMessage }
        } else {
