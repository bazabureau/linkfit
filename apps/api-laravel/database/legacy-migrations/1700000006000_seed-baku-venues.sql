...
-- Tennis is a first-class supported sport. Add tennis-specific courts only
-- at venues that are tennis-first in the public catalog.
INSERT INTO courts (venue_id, sport_id, name, hourly_price_minor, currency)
SELECT v.id, s.id, 'Tennis Court 1', price.amount, 'AZN'
FROM (VALUES
  ('Baku Tennis Club',     7000),
  ('Baku Tennis Academy',  7500)
) AS price(venue_name, amount)
JOIN venues v ON v.name = price.venue_name
JOIN sports s ON s.slug = 'tennis'
ON CONFLICT DO NOTHING;
