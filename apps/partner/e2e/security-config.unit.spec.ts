...
/**
 * Static guards for the P2/P3 security + SEO config. These read the committed
 * config/source files (no server needed) so a future edit that weakens the CSP,
 * drops the https assertion, or re-enables crawling fails the suite.
 *
...
test.describe("next.config CSP / security headers (P2)", () => {
...
test.describe("api.ts https-in-production assertion (P2)", () => {
  ...
  test("throws when NEXT_PUBLIC_API_URL is missing or non-https in prod", () => {
    ...
    // Build phase is exempted so the build itself doesn't trip.
    expect(api).toContain('IS_BUILD_PHASE');
  });
