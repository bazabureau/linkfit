...
// Loaded from `src/proxy.ts` exporting `proxy`. Redirects requests without the
// shared httpOnly access cookie to /login — defense-in-depth on top of the
// server-side 401s the API already enforces. Middleware runs server-side, so it
// can read the httpOnly cookie that JS cannot.
//
